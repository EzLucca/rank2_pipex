		...
		if (!cmd[0] || access(cmd[0], F_OK) == -1)
		{
			...
			if ((!path || !path[0] )/* && access(path, F_OK) == -1*/)
			{
				if (ft_strchr(argv[i], '/'))
					...
					// ft_dprintf(STDERR_FILENO, "teste\n");
...
int	get_files(t_pipex *pipex)
{
	...
	pipex->file_fd[1] = open(pipex->av[pipex->ac - 1], O_RDWR | O_CREAT | O_TRUNC, 0644);
...
void	ft_check_args(t_pipex *pipex, int argc, char **argv)
{
	...
	if (/*ft_strncmp(argv[0],"pipex_bonus", ft_strlen(argv[0])) || */ argc < 5)
	{
